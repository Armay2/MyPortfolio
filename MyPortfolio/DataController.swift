...
class DataController: ObservableObject {
    ...
    func hasEarned(award: Award) -> Bool {
        ...
        case "interest":
            // returns true if they added a certain number of interests
            let fetchRequest = Interest.fetchRequest()
            let awardCount = count(for: fetchRequest)
            return awardCount >= award.value
        case "doing":
            // returns true if they doing a certain number of interests
            let fetchRequest = Interest.fetchRequest()
            fetchRequest.predicate = NSPredicate(format: "isDoing == true")
            let awardCount = count(for: fetchRequest)
            return awardCount >= award.value
        case "tags":
